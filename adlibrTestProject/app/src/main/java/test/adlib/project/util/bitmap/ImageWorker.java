...
package test.adlib.project.util.bitmap;
...
public abstract class ImageWorker {
    ...
    /**
     * Load an image specified by the data parameter into an ImageView (override {@link ImageWorker#processBitmap(Object)} to define the processing logic). A memory and disk cache will be used if an {@link ImageCache} has been added using
     * {@link ImageWorker#addImageCache(android.support.v4.app.FragmentManager, ImageCache.ImageCacheParams)}. If the image is found in the memory cache, it is set immediately, otherwise an {@link AsyncTask} will be created to asynchronously load the bitmap.
     *
     * @param data
     ...
     * @param listener
     *            A listener that will be called back once the image has been loaded.
     */
    ...
    public interface OnImageLoadedListener {
         ...
         * 
         * @param success
         *            True if the image was loaded successfully, false if there was an error.
         */
        void onImageLoaded(boolean success);
    }

    /**
     * A custom Drawable that will be attached to the imageView while the work is in progress. Contains a reference to the actual worker task, so that it can be stopped if a new binding is required, and makes sure that only the last started worker process can bind its result, independently of the
     * finish order.
     */
